...
public class T09_equals_hashcodeTest {
	 ...
	 * 
	 * HashSet, HashMap, HashTable과 같은 객체들을 사용할 경우 객체가 서로 같은지를 서로 비교하기 위해 equals()메서드와 hashCode()메서드를 호출한다.
	 * 그래서 객체가 서로 같은지 여부를 결정하려면 두 메서드를 재정의 해야 한다.
	 * HashSet, HashMap, HashTable 에서는 객체가 같은지 여부를 데이터를 추가할 때 검사한다.
	 * 
	 ...
	 * 		즉, 객체a, b가 같다면 a.equals(b)와 b.equals(a) 둘 다 true이어야 한다.
	 ...
	 * 
	 * - hashCode()메서드에서 사용하는 "해싱 알고리즘" 에서 서로 다른 객체에 대하여 같은 hashCode값을 만들어 낼 수 없다. 그래서 객체가 같지 않더라도 hashCode가 같을 수 있다.
	 */
