...
/**
 * Thread의 stop()메서드를 호출하면 쓰레드가 바로 멈춘다. 이때 사용하던 자원을 정리하지 못하고 프로그램이 종료되어서 나중에
 * 실행되는 프로그램에 영향을 줄 수 있다. 그래서 현재 stop()메서드는 비추천(deprecated) 되어 있다.
 */
...
class ThreadStopEx2 extends Thread {
	...
	public void run() {
		/*
		// 방법 1 : sleep()메서드나 join()메서드 등을 사용 했을 때 interrupt()메서드를 호출하면
		...
		while(true) {
			...
			// 검사방법 1 : interrupt()메서드가 호출되었는지 검사
