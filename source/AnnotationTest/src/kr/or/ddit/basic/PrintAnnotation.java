 ...
 * annotation에 대하여
 *
 * 프로그램 소스코드 안에 다른 프로그램을 위한 정보를 미리 약속된 형식으로 포함시킨것(JDK1.5부터) 주석처럼 프로그래밍 언어에 영향을
 * 미치지 않으면서도 다른 프로그램에게 유용한 정보를 제공함 
 * 종류: 1. 표준 에네테이션(주로 컴파일러에게 유용한 정보를 제공하기 위한 에너테이션) 
 * 		2. 메타 에너테이션(에너테이션을 위한 에너테이션, 즉 에너테이션을 정의할 때 사용함.)
 *
 * 에너테이션 타입 정의하기
 * 
 * @interface 에너테이션 이름{
...
@Retention(RetentionPolicy.RUNTIME) // annotation이 유지 되는 기간(CLASS 기본값)
